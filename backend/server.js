...
try {
  ...
  console.warn('[security] express-rate-limit not installed - skipping rate limiting');
}
...
if (isProduction) {
  ...
  const missing = required.filter((k) => !process.env[k] || process.env[k].length < (k === 'JWT_SECRET' ? 32 : 1));
  if (missing.length) {
    throw new Error(`Missing/invalid production env vars: ${missing.join(', ')}`);
  }
}
