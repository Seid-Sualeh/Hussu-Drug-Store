    ...
    return (
      <div className="dashboard-page">
        <div className="alert alert-danger">
          {loadError ||
            "Could not load dashboard. Ensure the backend is running and the database is configured."}
        </div>
        <p className="text-muted small">
          Contact your administrator to initialize the database.
        </p>
      </div>
    );
  }
    ...
    <div className="dashboard-page">
      <div className="dashboard-header">
        <div>
          <h1>Dashboard</h1>
          <p>
            Quick answers for stock, expiry, profit, categories, suppliers &
            shelves
          </p>
        </div>
      ...
      <div className="dashboard-grid-2">
        ...
        <div className="dashboard-panel">
          ...
          <div className="expiry-legend">
            ...
            <span>
              <span className="legend-dot orange" /> Expiring Soon (≤180 days)
            </span>
            <span>
              ...
              <tbody>
                {expiryAlerts.slice(0, 12).map((row) => (
                  <tr key={row.id} className={`expiry-row ${row.status}`}>
                    ...
                    <td>
                      <span
                        className={`pill-badge ${row.status === "expired" ? "yes" : row.status === "expiring" ? "yes" : "no"}`}
                      >
                        {row.status === "expired"
                          ? "Expired"
                          : row.status === "expiring"
          ...
          <Link
            to="/expiry-alerts"
            className="small fw-semibold text-success mt-2 d-inline-block"
          >
        ...
        <div className="dashboard-panel">
          ...
          <p className="text-muted small mb-3">
            Based on min limit, max limit & current quantity
          </p>
          ...
          <div className="mt-3 d-flex gap-3 flex-wrap small">
            <span>
              <strong>{stockChart.underStock}</strong> Under Stock
            </span>
            <span>
              <strong>{stockChart.normal}</strong> Normal
            </span>
            <span>
              <strong>{stockChart.overStock}</strong> Over Stock
            </span>
            <span>
              <strong>{stockChart.outOfStock}</strong> Out of Stock
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="dashboard-grid-2">
        ...
        <div className="dashboard-panel">
          ...
          <p className="text-muted small mb-2">
            Profit = (Sell − Buy) × Qty | Profit % = (Profit ÷ Buy) × 100
          </p>
      ...
      <div className="dashboard-grid-3">
        ...
        >
          ...
          <div className="dashboard-panel">
            ...
            <p className="text-muted small mb-3">
              Which supplier gives most products?
            </p>
          ...
          <div className="dashboard-panel">
            ...
            <p className="text-muted small mb-3">
              Medicines below min limit or out of stock
            </p>
                ...
                urgentReorder.map((m) => (
                  <div key={m.id} className="urgent-item">
                    ...
                    <span className="urgent-badge">
                      {m.qty === 0 ? "Out of Stock" : "Under Stock"}
                    </span>
                  </div>
                ))
              )}
            ...
            <Link
              to="/inventory?stockStatus=under"
              className="small fw-semibold text-success mt-2 d-inline-block"
            >
              View in inventory →
            </Link>
        ...
        <div className="dashboard-panel">
          ...
          <p className="text-muted small mb-3">
            Which shelf contains which medicine?
          </p>
          <div className="shelf-grid">
            {shelfTracking.map((s) => (
              <div
                ...
                title={s.medicines?.map((m) => m.name).join(", ")}
              >
